...
import type { ProductApiItem } from '../types/backend';
import SAMPLE_BOOKS from '../mocks/books';

function mapToBook(item: ProductApiItem, index = 0): Book {
  const productId = item._id ?? index + 1;
  const id = typeof productId === 'number' ? productId : Number(productId) || index + 1;

  return {
    id,
    cover: item.imageUrl && item.imageUrl.length > 0 ? item.imageUrl : '/placeholder-cover.png',
    title: item.name,
    author: item.category ?? 'Unknown Author',
    price: item.price,
    rating: 0,
...
export async function getBooks(): Promise<Book[]> {
  ...
  try {
    ...
    if (res?.data) {
      const items: ProductApiItem[] = Array.isArray(res.data) ? res.data : res.data.items ?? [];
